...
@section('content')
    <form action="{{ isset($practicum) ? route('practicums.update', $practicum) : route('practicums.store') }}"
          method="POST">
        @isset($practicum)
            @method('PUT')
        @endisset
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-4">
                <input type="text" class="form-control @error('name') is-invalid @enderror" name="name"
                       id="name" value="{{ old('name', $practicum['name'] ?? '') }}">
                @error('name')
                <span class="invalid-feedback">
                    <strong>{{ $message }}</strong>
                </span>
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-4">
                <input type="text" class="form-control @error('year') is-invalid @enderror" name="year"
                       id="year" value="{{ old('year', $practicum['year'] ?? '') }}">
                @error('year')
                <span class="invalid-feedback">
                        <strong>{{ $message }}</strong>
                    </span>
                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive">
                            <table class="simple-table" id="module-table">
                                ...
                                <tbody>
                                @if(is_array(old('modules')) or isset($practicum))
                                    @foreach(old('modules', $practicum['modules'] ?? '') as $i => $module)
                                        <tr>
                                            <td>
                                                {{--                                                @if(isset($module['id']))--}}
                                            ...
                                            <td>
                                                <div class="col-sm-12 text-center p-0">
                                                    <input type="text"
                                                           ...
                                                           value="{{ $module['grade_publish_date'] }}"
                    ...
                    <div class="card-body">
                        <div class="table-responsive" style="overflow: visible !important;">
                            <table class="simple-table" id="assessment-table">
                                ...
                                <tbody>
                                @if(is_array(old('assessments')) or isset($practicum))
                                    @foreach(old('assessments', $practicum['assessments'] ?? '') as $i => $assessment)
                                        <tr>
                                            <td>
                                                {{--                                                @if($assessment['id'] != null)--}}
                                                <input type="hidden" name="assessments[{{$i}}][id]"
                                                       value="{{ $assessment['id'] ?? ''}}">
                                                {{--                                                @endif--}}
                                                <input type="text"
                                            ...
                                            <td>
                                                <select
                                                    ...
                                                    @foreach($answer_types as $answer_type)
                                                        <option
                                                            {{ ($assessment['answer_type'] == $answer_type->code) ? 'selected' : '' }}
                                            ...
                                            <td>
                                                <div class="custom-control custom-checkbox">
                                                    <input type="checkbox" class="custom-control-input"
                                                           ...
                                                           name="assessments[{{$i}}][isOnline]" {{ ($assessment['isOnline'] == 'OPTYS') ? 'checked' : '' }}>
    ...
    <script type="text/javascript">
        ...
        function add_module() {
            $(function () {
                ...
                let newRow =
                    '<tr>' +
                    '   <td>' +
                    // '       <input type="hidden" class="form-control input" name="modules[' + i + '][id]">' +
        ...
        function add_assessment() {
            ...
            let newRow =
                '<tr>' +
                '   <td>' +
                // '       <input type="hidden" class="form-control input" name="assessments[' + i + '][id]">' +
